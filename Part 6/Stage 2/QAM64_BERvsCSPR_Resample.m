...
xlabel('CSPR [dB]');
ylabel('BER');
title('64QAM: BER as a function of CSPR for different sps values')
grid on;

hold on
BER_plot2 = semilogy(CSPR_dB,BER_matrix(2,:),'-ob');
BER_plot4 = semilogy(CSPR_dB,BER_matrix(3,:),'-oc');
BER_plot4 = semilogy(CSPR_dB,BER_matrix(4,:),'-oy');
BER_plot5 = semilogy(CSPR_dB,BER_matrix(5,:),'--or');
BER_plot6 = semilogy(CSPR_dB,BER_matrix(6,:),'-ok');

legend('2.5 dsp sps','3 dsp sps','3.5 dsp sps','4 dsp sps','5 dsp sps','6 dsp sps');

figure(2)
constellation_title = '64QAM: Before The Decision (sps=6 before KK)';  
display_constellation_64QAM(downsampled_signal_3,constellation_title)
